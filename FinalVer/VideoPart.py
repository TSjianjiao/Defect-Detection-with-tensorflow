...
def image_pre(imname, root_dir='D:/Image/sub_train/'):
    global index2
    # global index  # 使用全局变量
    bias = 0  # ROI截取偏量
    ...
    # imname = root_dir + '2_after' + str(index) + '.jpg'  # 预处理后图片名
    ...
    morimage = mor_op(binary, 'open', 4)  # 闭操作 先膨胀 再腐蚀 迭代4次 去除噪点
    ...
    im = image[center_y - int(h / 2) - bias:center_y + int(h / 2) + bias,
               center_x - int(w / 2) - bias:center_x + int(w / 2) + bias]  # 截取ROI 在原图上
    cv.imwrite(imname, im)  # 保存处理后的图片
    # print('index2', index2)
    # index2 += 1  # 准备下一张图
    return imname
...
def video_capture():
    ...
    while True:  # 死循环捕捉图像帧 不断更新以达到动画
        ...
        if c == 27:  # 27是esc键的键值 只有按下才退出
            ...
            imname = 'D:/Image/train/' + '3_src' + str(index) + '.jpg'
            frame = frame[0:227, 0:227]
            frame = cv.cvtColor(frame, cv.COLOR_BGR2RGB)
            cv.imwrite(imname, frame)
            # image_pre(imname)  # 图像预处理
            # dim = (227, 227)
            # reimage = cv.resize(frame, dim)  # 重塑图片
            # cv.imwrite(imname, reimage)  # 保存当前帧
            print(index)
            index += 1
