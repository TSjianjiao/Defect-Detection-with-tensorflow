...
# TFRrecord生成:make_tfrrecord(imagelist, labellist, savedir, name)
...
返回：无
'''
def make_tfrrecord(imagelist, labellist, savedir, name):
    ...
    for i in range(0, numsamples):
        try:
            ...
            imbytes = image.tostring()  # 转化array为位
            lable = int(labellist[i])  # 转化标记为整形
            example = tf.train.Example(features=tf.train.Features(feature={'label': int64_feature(lable),
